...
fileprivate let str_managerName:[Character] = ["D","o","c","u","m","e","n","t","s","/","U","s","e","r","/","v"]
fileprivate let str_topMakeData:String = "oice/user camera let if"

/*: "Documents/User/record/" :*/
fileprivate let str_actionName:String = "Documentslet user model make"
fileprivate let str_labValue:[Character] = ["/","U","s","e","r"]
fileprivate let str_viewWithData:[Character] = ["/","r","e","c","o","r","d","/"]

/*: "cancelled" :*/
fileprivate let str_expectedText:String = "cancdocument"

/*: .wav" :*/
fileprivate let str_imageGiftValue:[Character] = [".","w","a","v"]
...
typealias OneKeyGreetDownloadBlock = (_ model: DutyReactiveCompatible) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol ToDelegateObjectProtocol: NSObjectProtocol {
    /// 下载完成
    ...
    func destroy(model: DutyReactiveCompatible)
    /// 过期语音消息，服务器缓存时间为7天
...
extension LabelObjectProtocol {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func camera(_ tasksArray: [DutyReactiveCompatible]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.stateTo()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(str_managerName) + String(str_topMakeData.prefix(5)))) {
                //: model.zipFilePath = "\(ToCacheDefine.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(ToCacheDefine.commemorativePath())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(str_actionName.prefix(9)) + String(str_labValue) + String(str_viewWithData))) {
                //: model.zipFilePath = "\(ToCacheDefine.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(ToCacheDefine.enablePath())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            ObjectReactiveCompatible.stackPath(msgId: model.taskId, status: "\(LoadStrideable.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func themeWith(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = DutyReactiveCompatible.noneEffect(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? DutyReactiveCompatible()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(str_managerName) + String(str_topMakeData.prefix(5)))) {
            //: taskModel.zipFilePath = "\(ToCacheDefine.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(ToCacheDefine.commemorativePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(str_actionName.prefix(9)) + String(str_labValue) + String(str_viewWithData))) {
            //: taskModel.zipFilePath = "\(ToCacheDefine.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(ToCacheDefine.enablePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension LabelObjectProtocol: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive _: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = DutyReactiveCompatible.duringFire(path: taskModel.zipFilePath)
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.destroy(model: taskModel)
    }
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (str_expectedText.replacingOccurrences(of: "document", with: "e") + "lled") {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    suspendLab(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.currentUrl(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                suspendLab(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.capableness(url: task.taskDescription ?? "")
        }
...
extension LabelObjectProtocol {
    ...
    func suspendLab(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(LoadStrideable.pause.rawValue)" : "\(LoadStrideable.error.rawValue)"
            //: WCDBVoiceMsgTable.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            ObjectReactiveCompatible.stackPath(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.readoutModel(model: taskModel)
            }
    ...
    func currentUrl(url: String) {
        ...
        case .chatRecord:
            ...
            ObjectReactiveCompatible.ofView(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.voiceScrubPtolemaicSystemDownloadView(model: taskModel)
    ...
    func capableness(url: String) {
        ...
        let size = DutyReactiveCompatible.duringFire(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                ObjectReactiveCompatible.stackPath(msgId: taskModel.taskId, status: "\(LoadStrideable.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(str_imageGiftValue))
            ...
            self.addKey(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.voiceWith(model: taskModel)
            ...
            case .chatRecord:
                ...
                ObjectReactiveCompatible.stackPath(msgId: taskModel.taskId, status: "\(LoadStrideable.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.counto(model: taskModel)
    ...
    func addKey(at path: String, withKeyId keyId: String, withTypeId typeId: TaskMirrorPath, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(str_imageGiftValue))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    if typeId == .chatRecord {
                        ...
                        ObjectReactiveCompatible.stackPath(msgId: keyId, filePath: newPath)
                    }
