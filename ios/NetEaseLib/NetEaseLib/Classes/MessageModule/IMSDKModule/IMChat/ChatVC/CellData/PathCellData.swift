...
fileprivate let str_showOnName:[UInt8] = [0x69,0x6e,0x69,0x74,0x28,0x63,0x6f,0x64,0x65,0x72,0x3a,0x29,0x20,0x68,0x61,0x73,0x20,0x6e,0x6f,0x74,0x20,0x62,0x65,0x65,0x6e,0x20,0x69,0x6d,0x70,0x6c,0x65,0x6d,0x65,0x6e,0x74,0x65,0x64]

/*: "[未知消息]" :*/
fileprivate let str_edgeTitle:[Character] = ["[","未","知","消","\u{606f}","]"]

/*: "extra" :*/
fileprivate let str_availableTitle:String = "extrindex"

/*: "</at> " :*/
fileprivate let str_iconContent:[Character] = ["<","/","a","t",">"," "]

/*: "#0084FF" :*/
fileprivate let str_toolValue:[Character] = ["#","0","0","8","4","F","F"]

/*: "#FFEE4B" :*/
fileprivate let str_numberValue:[Character] = ["#","F","F","E","E"]
fileprivate let str_plusData:String = "4Bmenu input title"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let str_hideValue:String = "<at>button make index button remove"
fileprivate let str_cellTitle:String = "]+?<"

/*: "jumpKey" :*/
fileprivate let str_imageTitle:[Character] = ["j","u","m","p","K","e","y"]

/*: "user" :*/
fileprivate let str_directionTitle:String = "lister"

/*: "uid" :*/
fileprivate let str_socialRawValue:[UInt8] = [0x64,0x69,0x75]

/*: "#16D073" :*/
fileprivate let str_byTitle:String = "cancel child library#16D073"

/*: "#F95644" :*/
fileprivate let str_partyControlAppTitle:[Character] = ["#","F"]
fileprivate let str_subContent:[Character] = ["9","5","6","4","4"]
...
//: @objcMembers public class PathCellData: TalkingChatMsgBaseCellData {
@objcMembers public class PathCellData: AppCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_showOnName, encoding: .utf8)!)
    }
    ...
    override public var msgModel: ConfinementModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (String(str_edgeTitle))
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = cost(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func cost(msgInfoModel: FactJsonModel) -> NSMutableAttributedString {
        ...
        guard dic[(str_availableTitle.replacingOccurrences(of: "index", with: "a"))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.blockOf() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(str_toolValue)))! : UIColor(hex: (String(str_numberValue) + String(str_plusData.prefix(2))))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(str_hideValue.prefix(4)) + "@[\\S\\s" + str_cellTitle.capitalized + "/at>"), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo["jumpKey"] == "user" {
                ...
                uid = jumpInfo[String(bytes: str_socialRawValue.reversed(), encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: str_socialRawValue.reversed(), encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func textPath() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(str_byTitle.suffix(7))))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(str_partyControlAppTitle) + String(str_subContent)))!
        }
