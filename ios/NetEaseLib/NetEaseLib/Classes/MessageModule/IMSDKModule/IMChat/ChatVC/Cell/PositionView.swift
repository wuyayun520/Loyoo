...
fileprivate let str_fillTitle:[UInt8] = [0x69,0x6e,0x69,0x74,0x28,0x63,0x6f,0x64,0x65,0x72,0x3a,0x29,0x20,0x68,0x61,0x73,0x20,0x6e,0x6f,0x74,0x20,0x62,0x65,0x65,0x6e,0x20,0x69,0x6d,0x70,0x6c,0x65,0x6d,0x65,0x6e,0x74,0x65,0x64]

/*: "move" :*/
fileprivate let str_colorName:[Character] = ["m","o","v","e"]
...
class PositionView: UIView {
	var nextHideOpen: Bool = false
	var doingSum: Double = -71.4
	var bagArray: [AnyHashable] = []
	var countimateDictionary: [AnyHashable: String] = [:]
	var dataEnable: Bool = true
	var impressionTotal: Double = -69.6
	var nameArray: [AnyHashable] = []
	var scopeLifestyleDictionary: [AnyHashable: String] = [:]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_fillTitle, encoding: .utf8)!)
    }
    ...
    func afterHandle(_ danumuMsgModel: AmbientHandyJSON?) {
		...
		if var requestValue = danumuMsgModel?.widthHeight { 
			if var streetSmartValue = danumuMsgModel?.widthHeight { 
		            if ((self.inputAssistantItem.trailingBarButtonGroups.count == 8) && (self.inputAssistantItem.leadingBarButtonGroups.count == 5)) && (self.layer.contentsRect.size.width != 1) {
		                //: SWIFT_CUSTOM_DANGER_File_Call
		                let saveData = UpwardlyView()
		
		            saveData.eventQuantity = streetSmartValue
		            saveData.viewUserDoing = { [self] contentViewSwitch in
		            self.dataEnable = contentViewSwitch
		            
		            var saveData = self.needStopAll
		            saveData = false
		            if saveData != self.dataEnable {
		                self.dataEnable = saveData
		            }
		            
		            return self.dataEnable
		            }
		            saveData.valueInterval = { [self] mSayMagnitude in
		            self.impressionTotal = mSayMagnitude
		            
		            var saveData = requestValue
		                saveData += 1
		                if Int(saveData) > -82 {
		                    saveData = saveData - 1
		                }
		            if saveData < self.impressionTotal {
		                self.impressionTotal = saveData
		            }
		            
		            return self.impressionTotal
		            }
		            saveData.nameArray = { [self] freeArray in
		            self.nameArray = freeArray
		            ...
		            self.scopeLifestyleDictionary = windowDictionary
		            
		            guard var value = self.scopeLifestyleDictionary as? [String: String] else {
		                return nil
		            }
		            return value
		            }
		                self.addSubview(saveData)
		            }
    ...
    @objc func metronymicTo(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(metronymicTo(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension PositionView: UIGestureRecognizerDelegate {
    ...
    func paradigmItemUnwishedRearFirst(for danumuMsgModel: AmbientHandyJSON?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = ElementView(model: danumuMsgModel)
        ...
        var frame = CGRect(x: a_blockValue, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if ConstraintLanguageManager.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = a_blockValue
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (String(str_colorName)))
...
//    }
    ...
    func auditoryImage(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                paradigmItemUnwishedRearFirst(for: model, isFirst: isFirst)
