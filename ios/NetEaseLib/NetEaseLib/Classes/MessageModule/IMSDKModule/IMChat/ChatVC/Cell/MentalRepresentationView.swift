...
/*: "icon_chat_lock" :*/
fileprivate let str_makeTitle:String = "icon_chguard call import app"
fileprivate let str_valueLabTitle:[Character] = ["a","t","_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let str_whiteValue:String = "icon_else data view"
fileprivate let str_randomName:String = "urpingd"

/*: "icon_chat_burned_shan" :*/
fileprivate let str_userData:String = "record section head selficon_"
fileprivate let str_textName:String = "view color model labelburned_"
fileprivate let str_countContent:String = "whitehan"

/*: "Already burned" :*/
fileprivate let str_extraValue:String = "Already color of"
fileprivate let str_primaryData:String = "BURNED"
...
// __CLOSE_PRINT__
//
//  MentalRepresentationView.swift
//  AbroadTalking
//
...
class MentalRepresentationView: ComparableViewCell {
	var contactEnable: Bool = false
	var leadInQuantity: Int = 6
    ...
    override func awakeFromNib() {
        ...
        super.awakeFromNib()
            ...
            if (lockImagV.inputViewController != nil) && (lockImagV.layer.isDoubleSided != true) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let addTransition = SectionView(frame: lockImagV.bounds)



            
            addTransition.bottomFreeClose = { [self] toolPriceDoing in
            self.contactEnable = toolPriceDoing
            
            return self.contactEnable
            }
            addTransition.groupMagnitude = { [self] awardSum in
            self.leadInQuantity = awardSum
            
            return self.leadInQuantity
            }
            addTransition.moveArray = { [self] targetMoonArray in
            self.timeArray = targetMoonArray
            ...
            return value
                ...
                lockImagV.addSubview(addTransition)
            }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        //: imag.alpha = 0.9
        imag.alpha = 0.9
        //: imag.image = UIImage.outsideText(name: "icon_chat_lock")
        imag.image = UIImage.outsideText(name: (String(str_makeTitle.prefix(7)) + String(str_valueLabTitle)))
    ...
    lazy var burnedImagV: PathErrorTalkingButton = {
        ...
        imgV.setBackgroundImage(UIImage.outsideText(name: (String(str_whiteValue.prefix(5)) + "chat_b" + str_randomName.replacingOccurrences(of: "ping", with: "ne"))), for: .normal)
        //: imgV.setImage(UIImage.outsideText(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.outsideText(name: (String(str_userData.suffix(5)) + "chat_" + String(str_textName.suffix(7)) + str_countContent.replacingOccurrences(of: "white", with: "s"))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(str_extraValue.prefix(8)) + str_primaryData.lowercased()).localized, for: .normal)
...
extension MentalRepresentationView {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)
        //: guard let imageCellData = data as? DocumentCellData else { return }
        guard let imageCellData = data as? DocumentCellData else { return }

        //: self.photoView.backgroundColor = .white
        self.photoView.backgroundColor = .white
        //: let intimatePhoto: InputTalkingModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        let intimatePhoto: InputTalkingModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            head(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            head(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: DocumentCellData, urlStr: String?, isMosaic: Bool) {
    func head(data: DocumentCellData, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    if isMosaic {
                        ...
                        self.photoView.image = UIImage.head(image, choppingBlock: 90)
                        //: } else {
                    } else {
            ...
            accentingMy(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.recordFinish(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func accentingMy(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.scoff(urlStr: urlStr) { [weak self] image in
