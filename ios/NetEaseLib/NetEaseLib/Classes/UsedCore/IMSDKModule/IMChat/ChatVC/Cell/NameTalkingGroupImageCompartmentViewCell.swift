...
fileprivate let str_dataTitle:[UInt8] = [0x69,0x6e,0x69,0x74,0x28,0x63,0x6f,0x64,0x65,0x72,0x3a,0x29,0x20,0x68,0x61,0x73,0x20,0x6e,0x6f,0x74,0x20,0x62,0x65,0x65,0x6e,0x20,0x69,0x6d,0x70,0x6c,0x65,0x6d,0x65,0x6e,0x74,0x65,0x64]

/*: "icon_chat_translate_nor" :*/
fileprivate let str_statusModeFreeName:[Character] = ["i","c","o","n","_","c","h","a","t","_","t","r"]
fileprivate let str_bottomTitle:String = "asize"
fileprivate let str_equalData:String = "of layer if return giftlate_nor"

/*: "icon_chat_translate_pre" :*/
fileprivate let str_sectionScreenCookieName:String = "icon_quote import if type title"
fileprivate let str_observerValue:String = "transindex"
fileprivate let str_succeedTitle:[Character] = ["a","t","e","_","p","r","e"]

/*: "icon_chat_miss" :*/
fileprivate let str_partyErrorValue:[Character] = ["i","c","o","n","_","c","h","a","t"]
fileprivate let str_photoData:[Character] = ["_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let str_arrayValue:String = "iccell"
fileprivate let str_allData:String = "var user to_yes"
...
// __CLOSE_PRINT__
//
//  NameTalkingGroupImageCompartmentViewCell.swift
//  AbroadTalking
//
...
class NameTalkingGroupImageCompartmentViewCell: ComparableViewCell {
    var equalImageView: UIImageView?

    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: PathCellData?
    var textData: PathCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_dataTitle, encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.outsideText(name: (String(str_statusModeFreeName) + str_bottomTitle.replacingOccurrences(of: "size", with: "ns") + String(str_equalData.suffix(8)))), for: .normal)
        //: btn.setImage(UIImage.outsideText(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.outsideText(name: (String(str_sectionScreenCookieName.prefix(5)) + "chat_" + str_observerValue.replacingOccurrences(of: "index", with: "l") + String(str_succeedTitle))), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(giftLabel), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension NameTalkingGroupImageCompartmentViewCell {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)
        //: textData = data as? PathCellData
        textData = data as? PathCellData
        ...
        //: let maxWidth = ImageMacroDefine.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = ImageMacroDefine.blazeAway() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.outsideText(name: (String(str_partyErrorValue) + String(str_photoData)))
            //: } else if textData.translateStatus == .MsgTranslate_succeed {
        } else if textData.translateStatus == .MsgTranslate_succeed {
            //: self.transShowView.stopAnimating()
            self.transShowView.stopAnimating()
            //: self.transStatusImagV.image = UIImage.outsideText(name: "icon_chat_yes")
            self.transStatusImagV.image = UIImage.outsideText(name: (str_arrayValue.replacingOccurrences(of: "cell", with: "on") + "_chat" + String(str_allData.suffix(4))))
        }
    ...
    func extended() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(stack(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func stack(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
