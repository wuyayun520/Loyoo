...
/*: "serialQueue" :*/
fileprivate let str_changeData:String = "seriborder"

/*: "fromUids" :*/
fileprivate let str_sessionModelValue:[UInt8] = [0x66,0x72,0x6f,0x6d,0x55,0x69,0x64,0x73]
...
let a_messageValue = 50

//: @objcMembers
@objcMembers
//: public class DBUserInfoManager: NSObject {
public class InfoReactiveCompatible: NSObject {
    ...
    private static var _instance: InfoReactiveCompatible?
    //: class func shared() -> DBUserInfoManager {
    class func colorButton() -> InfoReactiveCompatible {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = DBUserInfoManager()
            _instance = InfoReactiveCompatible()
            // 添加退出账号通知
            //: NotificationCenter.default.addObserver(_instance!,
            NotificationCenter.default.addObserver(_instance!,
                                                   //: selector: #selector(accountLogout),
                                                   selector: #selector(theLoad),
    ...
    @objc private func theLoad() {
        //: DBUserInfoManager.destroy()
        InfoReactiveCompatible.m()
    }
}
...
extension InfoReactiveCompatible {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([CountMediumCacheModel]?, Error?) -> Void) {
    class func alert(with userIdsArr: [String], completion: @escaping ([CountMediumCacheModel]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if InfoReactiveCompatible.colorButton().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(InfoReactiveCompatible.colorButton().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            InfoReactiveCompatible.colorButton().needReqUidsSet = nil
        }
        ...
        repeat {
            ...
            star(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func addEqual(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (str_changeData.replacingOccurrences(of: "border", with: "a") + "lQueue"))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    //: DoingSheRequestTool.req_queryStatus(toUids: requestUidsStr) { _, list in
                    DoingSheRequestTool.linguisticContext(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
...
extension InfoReactiveCompatible {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> CountMediumCacheModel? {
    class func pointOfReferenceWearerClickCachedGetSubstance(targetId: String) -> CountMediumCacheModel? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return AmmunitionInfoTable.popEqual(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: CountMediumCacheModel?) {
    class func cropFollowPavementStreetwise(with userInfo: CountMediumCacheModel?) {
        ...
        let model = InfoReactiveCompatible.pointOfReferenceWearerClickCachedGetSubstance(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        AmmunitionInfoTable.statusShow(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: a_messageNetData, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func showKeyTime(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = InfoReactiveCompatible.pointOfReferenceWearerClickCachedGetSubstance(targetId: targetId) else { return }
    ...
    class func trailhead(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = InfoReactiveCompatible.pointOfReferenceWearerClickCachedGetSubstance(targetId: targetId) else { return }
    ...
    class func toStreetwise(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        AmmunitionInfoTable.vocalizationInput(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        InfoReactiveCompatible.colorButton().reqedUidsSet.remove(targetUid)
    }
}
...
extension InfoReactiveCompatible {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([CountMediumCacheModel]?, Error?) -> Void) {
    private class func star(with userIdsArr: [String], completion: @escaping ([CountMediumCacheModel]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = AmmunitionInfoTable.breakUpSomebody(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if InfoReactiveCompatible.colorButton().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            InfoReactiveCompatible.blockTo(with: noDBUidArr)
        }
    }
    ...
    private class func blockTo(with userIdsArr: [String]) {
        ...
        InfoReactiveCompatible.colorButton().reqedUidsSet = InfoReactiveCompatible.colorButton().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: DoingSheRequestTool.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        DoingSheRequestTool.exceptResign(param: [String(bytes: str_sessionModelValue, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                InfoReactiveCompatible.colorButton().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            InfoReactiveCompatible.retardationPath(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: CountMediumCacheModel] = [:]
            var retInfoDict: [String: CountMediumCacheModel] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: a_messageNetData, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [CountMediumCacheModel]) {
    private class func retardationPath(userInfoArr: [CountMediumCacheModel]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            AmmunitionInfoTable.statusShow(aWrap)
        }
    }
}
