...
fileprivate let str_shadowErrorValue:[UInt8] = [0x69,0x6e,0x69,0x74,0x28,0x63,0x6f,0x64,0x65,0x72,0x3a,0x29,0x20,0x68,0x61,0x73,0x20,0x6e,0x6f,0x74,0x20,0x62,0x65,0x65,0x6e,0x20,0x69,0x6d,0x70,0x6c,0x65,0x6d,0x65,0x6e,0x74,0x65,0x64]

/*: "%02ld:%02ld:%02ld" :*/
fileprivate let str_turnData:[Character] = ["%","0","2","l","d",":","%","0","2"]
fileprivate let str_fireData:[Character] = ["l","d",":","%","0","2","l","d"]

/*: "icon_mini_voice_end" :*/
fileprivate let str_titleData:[UInt8] = [0x69,0x63,0x6f,0x6e,0x5f,0x6d,0x69,0x6e,0x69,0x5f,0x76,0x6f,0x69,0x63,0x65,0x5f,0x65,0x6e,0x64]

/*: "icon_mini_voice_calling" :*/
fileprivate let str_textViewOfData:String = "icon_mview view type let kit"
fileprivate let str_ofResignTitle:String = "method upvoice_cal"
fileprivate let str_listData:String = "lifileg"

/*: "#F95151" :*/
fileprivate let str_labelValue:[Character] = ["#","F","9","5","1","5","1"]

/*: "#34C759" :*/
fileprivate let str_sourceData:String = "make text#34C759"

/*: "End of call" :*/
fileprivate let str_titleName:String = "End of cvar to to view to"
fileprivate let str_modelData:String = "messagel"
...
class CellMiniView: LabelMiniView {
	var styleMagnitude: Double = 27.2
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_shadowErrorValue, encoding: .utf8)!)
    }
    ...
    override func sortExit() {
        ...
        everyTextStatus(status: .end)
        //: super.dismissWhenCallEnd()
        super.sortExit()
    
            if (!bgView.isOpaque) && (bgView.intrinsicContentSize.width == 304.41) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let convertLet = TimePeriodView()
            convertLet.bagOpen = pointAnimView.isPlayAnimation

            convertLet.labCount = { [self] detailInterval in
            self.styleMagnitude = detailInterval
            
            return self.styleMagnitude
            }
                bgView.addSubview(convertLet)
            }
...
extension CellMiniView {
    /// 构建音频小窗口
    //: static func buildVoiceMiniView() -> TalkingVoiceMiniView {
    static func vocalisationButton() -> CellMiniView {
        //: let view = TalkingVoiceMiniView(frame: .zero)
        let view = CellMiniView(frame: .zero)
...
extension CellMiniView {
    ...
    private func thirdParty() {
        ...
        everyTextStatus(status: .calling)
    }

    /// 刷新视图
    /// - Parameter status: 通话状态
    //: private func refreshVoiceView(status: MiniWindowStatus) {
    private func everyTextStatus(status: OccurrentWindowStatus) {
        //: self.currStatus = status
        self.currStatus = status
        //: let iconStr = (status == .end) ? "icon_mini_voice_end":"icon_mini_voice_calling"
        let iconStr = (status == .end) ? String(bytes: str_titleData, encoding: .utf8)! : (String(str_textViewOfData.prefix(6)) + "ini_" + String(str_ofResignTitle.suffix(9)) + str_listData.replacingOccurrences(of: "file", with: "n"))
        //: self.statusIcon.image = UIImage.outsideText(name: iconStr)
        self.statusIcon.image = UIImage.outsideText(name: iconStr)
        //: self.timeLab.textColor = (status == .end) ? UIColor(hex: "#F95151"):UIColor(hex: "#34C759")
        self.timeLab.textColor = (status == .end) ? UIColor(hex: (String(str_labelValue))) : UIColor(hex: (String(str_sourceData.suffix(7))))
        //: if status == .end {
        if status == .end {
            //: self.timeLab.text = "End of call".localized
            self.timeLab.text = (String(str_titleName.prefix(8)) + str_modelData.replacingOccurrences(of: "message", with: "al")).localized
        }
    }
}
