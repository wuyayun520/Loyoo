...
fileprivate let str_textValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class CountTrackView: UIView {
    var equivalentImageView: UIImageView?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_textValue.reversed(), encoding: .utf8)!)
    }
...
extension CountTrackView {
    ...
    override func hitTest(_ point: CGPoint, with _: UIEvent?) -> UIView? {
        ...
        for subview in self.subviews {
            ...
            let itemview: ModifyReactiveCompatible = subview as! ModifyReactiveCompatible
            ...
            if #available(iOS 11.0, *) {
                ...
                if itemview.avatarImage.frame.contains(point) {
                    //: LimitPushManager.share.func__pushToUserDetailVC(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    LimitPushManager.share.funcToStuffDetailUtilizerUid(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    //: break
                    break
                }
                //: } else {
    ...
    func terraceFirst(model: EventHandyJSON) {
        //  1.相同礼物时则连击
        //: for  subview in self.subviews {
        for subview in self.subviews {
            //: let itemview: TalkingGiftTrackItemView = subview as! TalkingGiftTrackItemView
            ...
            if bindBy(giftMsgModel: itemview.giftMsgModel, withGiftMsgModel: model) {
                //: itemview.comboHitWith(giftMsgModel: model)
                itemview.accommodateModel(giftMsgModel: model)
                //: itemview.setGiftMsgModel(model: model)
                itemview.kindJuxtaposePhone(model: model)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            let subView: ModifyReactiveCompatible? = self.viewWithTag(tag) as! ModifyReactiveCompatible?
            //: if subView == nil {
            if subView == nil {
                //: self.addNewGiftDanumWith(giftMsgModel: model, atIndex: index)
                self.clip(giftMsgModel: model, atIndex: index)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            if index == 1 || index == 2 {
                ...
                let subView1: ModifyReactiveCompatible = self.viewWithTag(tag) as! ModifyReactiveCompatible
                ...
                subView1.animationVideoCopernicanSystemImage(frame: frame)
    ...
    func bindBy(giftMsgModel: EventHandyJSON, withGiftMsgModel: EventHandyJSON) -> (Bool) {
...
//        subView.showUserInfoViewBlock   = self.showUserInfoViewBlock
        ...
        subView.kindJuxtaposePhone(model: giftMsgModel)
        //: self.addSubview(subView)
        self.addSubview(subView)

        //: subView.showGiftAnimate(model: giftMsgModel, animate: true)
        subView.commentAnimate(model: giftMsgModel, animate: true)
    }
}
