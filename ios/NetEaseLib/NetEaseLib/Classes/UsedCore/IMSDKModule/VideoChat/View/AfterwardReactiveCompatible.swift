...
fileprivate let str_fieldTitle:[Character] = ["T","R","T","C"," ","视"]
fileprivate let str_coinEqualValue:[Character] = ["频","\u{901a}","话","向","S","D","K","发","送"]
fileprivate let str_endName:String = "采集\u{7684}视"

/*: "LIVE 直播向SDK发送采集的视频帧图片:  :*/
fileprivate let str_centerValue:String = "return typeLIVE 直"
fileprivate let str_sizeName:String = "送采集的视\u{9891}"
fileprivate let str_giftReplyName:[Character] = ["帧","图","片",":"," "]
...
// __CLOSE_PRINT__
//
//  AfterwardReactiveCompatible.swift
//  AbroadTalking
//
...
class AfterwardReactiveCompatible: UIView {
    var discontinueImageView: UIImageView?
...
extension AfterwardReactiveCompatible {
    /// 开启本地预览
    /// - Parameters:
    ///   - isSmallWindow: 是否自己在小窗口
    ///   - livePusher: 是否用tx推流
    //: func startPreView(isSmallWindow: Bool, livePusher: TXLivePush? = nil) {
    func viewClick(isSmallWindow: Bool, livePusher: TXLivePush? = nil) {
    ...
    func galleryPrice(isFront _: Bool) {
        //: guard SenseTime_Use == true else { return }
        guard a_keyPostText == true else { return }
        //: guard self.captureView.stCamera != nil else {
        guard self.captureView.stCamera != nil else {
...
extension AfterwardReactiveCompatible: STFilterViewDelegete {
    // TRTC推流
    ...
    func stVideoBeautyView(_: STFilterView!, didProcessVideoSampleBuffer sampleBuffer: CVPixelBuffer!) {
        ...
        if trtcCallCount >= logReportInterval {
            ...
            if let image = upwardlyCorner(sampleBuffer) {
                //: UploadLogTool.writeLog(msg: "TRTC 视频通话向SDK发送采集的视频帧图片: \(image.size)")
                TitleMediumLogTool.commitMedium(msg: (String(str_fieldTitle) + String(str_coinEqualValue) + str_endName + "频帧\u{56fe}片: ") + "\(image.size)")
            }
            //: trtcCallCount = 0
            trtcCallCount = 0
        }
        ...
        if livePusher != nil {
            ...
            if liveCallCount >= logReportInterval {
                ...
                if let imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
                    ...
                    if let image = upwardlyCorner(imageBuffer) {
                        //: UploadLogTool.writeLog(msg: "LIVE 直播向SDK发送采集的视频帧图片: \(image.size)")
                        TitleMediumLogTool.commitMedium(msg: (String(str_centerValue.suffix(6)) + "播向SDK发" + str_sizeName + String(str_giftReplyName)) + "\(image.size)")
                    }
                }
    ...
    func upwardlyCorner(_ imageBuffer: CVImageBuffer) -> UIImage? {
