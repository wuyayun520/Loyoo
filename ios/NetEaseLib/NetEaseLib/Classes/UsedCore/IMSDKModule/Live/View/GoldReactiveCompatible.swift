...
/*: "99+" :*/
fileprivate let str_labTitle:[Character] = ["9","9","+"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_textBottomOutsideData:[UInt8] = [0x69,0x6e,0x69,0x74,0x28,0x63,0x6f,0x64,0x65,0x72,0x3a,0x29,0x20,0x68,0x61,0x73,0x20,0x6e,0x6f,0x74,0x20,0x62,0x65,0x65,0x6e,0x20,0x69,0x6d,0x70,0x6c,0x65,0x6d,0x65,0x6e,0x74,0x65,0x64]

/*: "#2DF2FF" :*/
fileprivate let str_equalName:String = "#action"
fileprivate let str_acceptValue:String = "DFsunF"

/*: "Live" :*/
fileprivate let str_userName:String = "Livehoney color"

/*: "img_home_shadow_icon" :*/
fileprivate let str_needData:String = "img_homeself universal"
fileprivate let str_bindValue:String = "_shapath height back clear if"

/*: "get json error" :*/
fileprivate let str_tempData:String = "get jsmessage value else level view"

/*: "#FF2348" :*/
fileprivate let str_conName:String = "view if#FF234"
fileprivate let str_keyUserData:[Character] = ["8"]

/*: "#35C759" :*/
fileprivate let str_insideTitle:[Character] = ["#","3","5","C","7","5","9"]

/*: "mfBean" :*/
fileprivate let str_mediumValue:String = "mfBeancell false extension then"

/*: "%.1f" :*/
fileprivate let str_storageFormalData:String = "%.1factual item view"

/*: "points\n≈ $" :*/
fileprivate let str_pathValue:String = "pointi"
fileprivate let str_managerValue:[Character] = ["n","t","s","\n","≈"," ","$"]

/*:   :*/
fileprivate let str_offName:[Character] = [" "]
...
public var a_screenContent: Int {
    //: return MeasurementAppManager.share.appConfigMode.countdown1 + MeasurementAppManager.share.appConfigMode.countdown2
    return MeasurementAppManager.share.appConfigMode.countdown1 + MeasurementAppManager.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class GoldReactiveCompatible: LabelMiniView {
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= MeasurementAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(a_screenContent - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                equalOf()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(MeasurementAppManager.share.loginUserMode.mf_bean) > 0 {
        if Double(MeasurementAppManager.share.loginUserMode.mf_bean) > 0 {
            //: setPointText(mfBean: 0.0)
            overdoButton(mfBean: 0.0)
        }
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_textBottomOutsideData, encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: PathErrorTalkingButton = {
        ...
        let image = UIImage.visualPropertyView(color: UIColor(hex: (str_equalName.replacingOccurrences(of: "action", with: "2") + str_acceptValue.replacingOccurrences(of: "sun", with: "2F")))!, size: CGSize(width: 8, height: 8))
        ...
        img.setTitle((String(str_userName.prefix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.outsideText(name: (String(str_needData.prefix(8)) + String(str_bindValue.prefix(4)) + "dow_icon"))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (String(str_tempData.prefix(6)) + "on error"))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        ...
        lab.font = UIFont.methodPlay(fontSize: 20)
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.textColor = UIColor(hex: "#FF2348")
        lab.textColor = UIColor(hex: (String(str_conName.suffix(6)) + String(str_keyUserData)))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(str_insideTitle)))
...
extension GoldReactiveCompatible {
    /// 构建视频小窗口
    //: static func buildLiveMiniView() -> TalkingLiveMiniView {
    static func dismissWith() -> GoldReactiveCompatible {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = GoldReactiveCompatible(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = SocketReactiveCompatible.shared.liveMiniPosition
    ...
    func equalOf() {
        ...
        liveStatusImgV.isHidden = !PropertyMManageressPushListener.giftShared().isLive
...
extension GoldReactiveCompatible {
    ...
    private func someToThird() {
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(pointchange(notification:)),
                                               selector: #selector(everyFrom(notification:)),
                                               //: name: LIVE_USER_POINT_CHANGE,
                                               name: a_videoDeviceData,
                                               //: object: nil)
                                               object: nil)
    }

    //: @objc private func pointchange(notification: NSNotification) {
    @objc private func everyFrom(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(str_mediumValue.prefix(6)))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        overdoButton(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func overdoButton(mfBean: Double) {
        //: let nowBean = MeasurementAppManager.share.loginUserMode.mf_bean + mfBean
        let nowBean = MeasurementAppManager.share.loginUserMode.mf_bean + mfBean
        //: MeasurementAppManager.share.loginUserMode.mf_bean = nowBean
        MeasurementAppManager.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((str_pathValue.replacingOccurrences(of: "point", with: "po") + String(str_managerValue)).localized) \(divisor)"
