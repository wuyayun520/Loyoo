...
fileprivate let str_itemContent:[UInt8] = [0x69,0x6e,0x69,0x74,0x28,0x63,0x6f,0x64,0x65,0x72,0x3a,0x29,0x20,0x68,0x61,0x73,0x20,0x6e,0x6f,0x74,0x20,0x62,0x65,0x65,0x6e,0x20,0x69,0x6d,0x70,0x6c,0x65,0x6d,0x65,0x6e,0x74,0x65,0x64]

/*: "Reply" :*/
fileprivate let str_directionTitle:String = "Replyview cookie agent"

/*: "#F95151" :*/
fileprivate let str_renderValue:[Character] = ["#","F","9","5","1","5","1"]
...
let a_nameMomentText = (a_limitRecordScreenName + 80.0)
...
class MeasurementView: UIView {
    ...
    var startDismissBlock: ((_ model: AdministratorReactiveCompatible) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: MeasurementView) -> Void)? // 完成移除视图Block
    //: private var currModel = TalkingConversationModel()
    private var currModel = AdministratorReactiveCompatible()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_itemContent, encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .thoughtImage(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .thoughtImage(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(str_directionTitle.prefix(5))).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(str_renderValue))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension MeasurementView {
    ...
    func momentProgressShow(_ model: AdministratorReactiveCompatible?) {
    ...
    func subAlpha(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -a_nameMomentText
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func dataConverter() {
        //: TalkingMessageNotificationManager.shared.clearData()
        ManagerEventReactiveCompatible.shared.transfer()
        // 在音视频界面，缩小通话窗口
        ...
        if currModel.chatType == .service {
            ...
            //: LimitPushManager.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            LimitPushManager.share.sharedMode(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func groupAction(gesture: UISwipeGestureRecognizer) {
...
extension MeasurementView {
    ...
    func subAdd() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.posit(urlStr: userInfo.headPic, placeImg: UIImage.tempWithGift(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.fileStr(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
    ...
    private func create() {
        ...
        self.frame = CGRect(x: 0, y: -a_nameMomentText, width: a_blockValue, height: a_nameMomentText)
    ...
    private func eyeContact() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(dataConverter))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(groupAction))
        //: swipe.direction = .up
        swipe.direction = .up
