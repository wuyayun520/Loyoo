...
/*: "App" :*/
fileprivate let str_theWillData:String = "else make string left indexApp"

/*: "blockList_ :*/
fileprivate let str_modelTitle:[Character] = ["b","l","o","c","k","L","i","s","t","_"]
...
class ModelAtListManager: NSObject {
    ...
    /// 加载拉黑列表
    //: func loadBlockList() {
    func numbereractionAfter() {
        //: let diskPath = getCachePath()
        let diskPath = luckSumoQuantityStraitAndNarrow()
    ...
    func upShared(userId: String) -> Bool {
    ...
    private func luckSumoQuantityStraitAndNarrow() -> String {
        //: let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent((String(str_theWillData.suffix(3))))
        ...
        let filePath = (appDirectoryPath as NSString).appendingPathComponent((String(str_modelTitle)) + "\(uid)")
        //: return filePath
        return filePath
    }
}
