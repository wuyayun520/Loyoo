...
/*: "midfield_img" :*/
fileprivate let str_leadingData:String = "midfimake"
fileprivate let str_layerTextValue:[Character] = ["d","_","i","m","g"]

/*: "icon_kong_kong_default" :*/
fileprivate let str_toData:String = "minion"
fileprivate let str_containerData:String = "max let path frame viewkong_"
fileprivate let str_layerValue:String = "DEFAULT"

/*: "page" :*/
fileprivate let str_makeLocalHiddenData:[UInt8] = [0x70,0x61,0x67,0x65]

/*: "uid" :*/
fileprivate let str_tabData:[UInt8] = [0x75,0x69,0x64]

/*: "list" :*/
fileprivate let str_tableTitle:String = "lcellt"
...
class ImageEndUserDataSource: PropertyViewController {
	var containerTotal: Int = 30
    ...
    private var postDataArr = [OneMeasurable]()
    ...
    override func viewDidLoad() {
        ...
        quoteImageView = UIImageView(frame: self.view.frame.insetBy(dx: CGFloat(Double(self.view.center.x)), dy: CGFloat(0)))
        if let quoteImageView = quoteImageView {
            self.quoteImageView?.image = UIImage(named: (str_leadingData.replacingOccurrences(of: "make", with: "el") + String(str_layerTextValue)))
            if (quoteImageView.isMultipleTouchEnabled) && (quoteImageView.clipsToBounds) {
                //: SWIFT_CUSTOM_DANGER
                self.view.addSubview(quoteImageView)
            }
        }
    
            if (collectionView.forLastBaselineLayout.center.x == 43.47) && (collectionView.convert(CGPoint(x: CGFloat(0), y: 0), to: collectionView.superview).y == 44.54) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let sizeGold = PhoneRateView()
            sizeGold.passAwayDoing = self.hideNavi

            sizeGold.shadowRoomQuantity = { [self] lodestarQuantity in
            self.containerTotal = lodestarQuantity
            
            var sizeGold = pageIndex
            sizeGold -= 1
            if sizeGold > self.containerTotal {
                self.containerTotal = sizeGold
            }
            
            return self.containerTotal
            }
                collectionView.addSubview(sizeGold)
            }
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: a_itemBarValue, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(GestureValueView.self, forCellWithReuseIdentifier: GestureValueView.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.outsideText(name: "icon_kong_kong_default")
        img.image = UIImage.outsideText(name: (str_toData.replacingOccurrences(of: "mini", with: "ic") + "_kong_" + String(str_containerData.suffix(5)) + str_layerValue.lowercased()))
...
extension ImageEndUserDataSource {
    //: private func headerRefresh() {
    private func startHeader() {
        //: pageIndex = 0
        pageIndex = 0
        //: load_listData()
        request()
    ...
    private func request() {
        ...
        let params = [String(bytes: str_makeLocalHiddenData, encoding: .utf8)!: pageIndex, String(bytes: str_tabData, encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        RequestTool.thrusting(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(str_tableTitle.replacingOccurrences(of: "cell", with: "is"))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = CustomMeasurable.deserialize(from: dict as? Dictionary) {
...
extension ImageEndUserDataSource: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: GestureValueView.className(), for: indexPath) as! GestureValueView
        //: if indexPath.row < self.postDataArr.count {
        if indexPath.row < self.postDataArr.count {
            //: let model = self.postDataArr[indexPath.row]
            let model = self.postDataArr[indexPath.row]
            //: cell.refreshItem(model)
            cell.cutWithoutPlace(model)
        }
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = MudraDetailVc(model: detailModel)
...
extension ImageEndUserDataSource: JXPagingViewListViewDelegate {
...
extension ImageEndUserDataSource {
    ...
    func controller(_ userModel: ToInfoModel) {
