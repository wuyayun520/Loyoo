...
/*: "id" :*/
fileprivate let str_segmentText:[Character] = ["i","d"]

/*: "reply" :*/
fileprivate let str_userFromText:String = "labelply"

/*: :  :*/
fileprivate let str_statusValue:String = ":load"
...
struct AppWithHandyJSON: HandyJSON {
    //: var cid: String?
    ...
    mutating func cancel() {
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if ConstraintLanguageManager.shared.direction == .leftToRight {
                ...
                tempContent = (str_userFromText.replacingOccurrences(of: "label", with: "re")).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (str_userFromText.replacingOccurrences(of: "label", with: "re")).localized
            }
        ...
        let sizeheight = Int(GoldView.workInNumber(text: tempContent, size: CGSize(width: a_blockValue - 70, height: CGFloat(MAXFLOAT)), font: UIFont.thoughtImage(type: .Regular, fontSize: 16), lineNumber: 0).height)
