...
fileprivate let str_deviceValue:[UInt8] = [0xd7,0xd4,0xd8,0xda,0xcf,0xd2,0xd4,0xd5,0x9b,0xda,0xce,0xcf,0xd3,0xd4,0xc9,0xd2,0xc1,0xda,0xcf,0xd2,0xd4,0xd5,0xe8,0xcf,0xda,0xcf,0xce,0xc8,0x9b,0xd2,0xc8,0x9b]

/*: "get location failed. error: :*/
fileprivate let str_viewRenderValue:[Character] = ["g","e","t"," ","l","o","c","a","t","i"]
fileprivate let str_intervalValue:[Character] = ["o","n"," "]
fileprivate let str_groupName:[Character] = ["f","a","i","l","e","d","."," ","e","r","r","o","r",":"]
...
class EqualLocationManager: NSObject {
    ...
    var locationType: TextStrideable?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: EqualLocationManager?
    //: class func shared() -> TalkingLocationManager {
    class func sinceRequest() -> EqualLocationManager {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = EqualLocationManager()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    func resultBlock(block: @escaping Locationblock) {
    ...
    func noWithBind(block: @escaping Authblock) {
    ...
    func hereViewConsultingService() -> Bool {
    ...
    func sizeOf() -> TextStrideable {
        //: switch locationPermission() {
        switch spaceWithinPermission() {
        //: case .notDetermined:
        case .notDetermined:
            //: return .firstEntry
            return .firstEntry
    ...
    func spaceWithinPermission() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: str_deviceValue.map{$0^187}, encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension EqualLocationManager: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(str_viewRenderValue) + String(str_intervalValue) + String(str_groupName)) + "\(error.localizedDescription)")
    }
}
