...
/*: "type" :*/
fileprivate let str_nextTitle:String = "typmake"

/*: "You are currently on a call, please try again later" :*/
fileprivate let str_buttonVideoValue:[UInt8] = [0x59,0x6f,0x75,0x20,0x61,0x72,0x65,0x20,0x63,0x75,0x72,0x72,0x65,0x6e,0x74,0x6c,0x79,0x20,0x6f,0x6e,0x20,0x61,0x20,0x63,0x61,0x6c,0x6c,0x2c,0x20,0x70,0x6c,0x65,0x61,0x73,0x65,0x20,0x74,0x72,0x79,0x20,0x61,0x67,0x61,0x69,0x6e,0x20,0x6c,0x61,0x74,0x65,0x72]

/*: "onCallResume" :*/
fileprivate let str_modeTitle:String = "result key super fill falseonCal"
fileprivate let str_userData:[Character] = ["l","R","e","s","u","m","e"]
...
// __CLOSE_PRINT__
//
//  MeasurementAppManager+VideoChat.swift
//  AbroadTalking
//
...
extension MeasurementAppManager {
    ...
    func cancelAction(info: [String: Any], situation: AdpressedChatSituation = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        //: let type = json["type"].intValue
        let type = json[(str_nextTitle.replacingOccurrences(of: "make", with: "e"))].intValue
        ...
        if type == 0 { // 视频
            ...
            vc = HumanActivityPartyDelegate()
            //: (vc as! TalkingVideoChatViewController).chatModel = TalkingVideoChatModel.getVideoModel(info: info, situation: situation)
            (vc as! HumanActivityPartyDelegate).chatModel = ImageChatModel.dismissPrivacy(info: info, situation: situation)
            //: } else {
        } else { // 音频
            ...
            (vc as! ToObjectProtocol).chatModel = ImageChatModel.dismissPrivacy(info: info, situation: situation)
        }

        //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: false, resumeData: [:])
        self.dormerWindow(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: false, resumeData: [:])
    }

    /// 接收到视频通话
    /// - Parameter info: 数据
    //: func recieve1v1TalkRequest(info: [String: Any]) {
    func roomLoad(info: [String: Any]) {
        //: handle1v1AudioRequest(info: info, resumeTalk: false)
        handleToSoliloquiseTalk(info: info, resumeTalk: false)
    }

    /// 恢复视频通话
    /// - Parameter info: 数据
    //: func recieve1v1TalkResumeRequest(info: [String: Any]) {
    func subjectMatterRequestInfo(info: [String: Any]) {
        //: handle1v1AudioRequest(info: info, resumeTalk: true)
        handleToSoliloquiseTalk(info: info, resumeTalk: true)
    }
}
...
extension MeasurementAppManager {
    ...
    private func handleToSoliloquiseTalk(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.isTalking {
        if !resumeTalk && SocketReactiveCompatible.shared.isTalking {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.episode(showMsg: String(bytes: str_buttonVideoValue, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if colorController()!.isKind(of: HumanActivityPartyDelegate.self) ||
            //: currentViewController()!.isKind(of: TalkingVoiceChatViewController.self) ||
            colorController()!.isKind(of: ToObjectProtocol.self) ||
            //: currentViewController()!.isKind(of: TalkingVideoInitivCallVC.self) {
            colorController()!.isKind(of: MeasurementRecognizerDelegate.self)
        {
            //: return
            return
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! HumanActivityPartyDelegate).chatModel = ImageChatModel.dismissPrivacy(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = ToObjectProtocol()
            ...
            (vc as! ToObjectProtocol).chatModel = ImageChatModel.dismissPrivacy(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(str_modeTitle.suffix(5)) + String(str_userData))].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.dormerWindow(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func dormerWindow(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = colorController() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = ImageMacroDefine.getWindow().rootViewController as! UITabBarController
            let rootVc = ImageMacroDefine.sumHungWindowObject().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(ToObjectProtocol.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(HumanActivityPartyDelegate.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            var currentChatVc: HumanActivityPartyDelegate?
            //: let pushChatVc = roomVC as! TalkingVideoChatViewController
            let pushChatVc = roomVC as! HumanActivityPartyDelegate

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVideoChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? HumanActivityPartyDelegate
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.contentRaw(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: ConstraintNavigationController.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! ConstraintNavigationController
            ...
            if topVc != nil, topVc!.isKind(of: CoincideThen.self) {
            ...
            var currentChatVc: ToObjectProtocol?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! ToObjectProtocol

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? ToObjectProtocol
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.observerOf(resumeData: resumeData)
                }
                //: return
                return
            }
