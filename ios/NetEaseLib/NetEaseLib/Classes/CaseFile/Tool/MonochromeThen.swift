...
fileprivate let str_textName:[Character] = ["A","p","p","R","e","s","o","u","r","c","e","s","V","e","r","s"]
fileprivate let str_pathTitle:String = "of"
fileprivate let str_addTitle:[Character] = ["o","n","K","e","y"]

/*: ".zip" :*/
fileprivate let str_labValue:String = ".zipindex var else super"

/*: "pic" :*/
fileprivate let str_appOfName:String = "pivoice"

/*: "nor" :*/
fileprivate let str_objectValue:[Character] = ["n","o","r"]

/*: "svga" :*/
fileprivate let str_defineName:[Character] = ["s","v","g","a"]

/*: "mp3" :*/
fileprivate let str_aspectTitle:String = "birthday3"

/*: "model" :*/
fileprivate let str_wrapValue:[Character] = ["m","o","d","e","l"]

/*: "@2x.png" :*/
fileprivate let str_giftGiveData:String = "@2"
fileprivate let str_objectName:String = "view extrax.png"

/*: "png" :*/
fileprivate let str_tempValue:[Character] = ["p","n","g"]
...
private let a_itemName = (String(str_textName) + str_pathTitle.replacingOccurrences(of: "of", with: "i") + String(str_addTitle))

//: enum SVGAType: String {
enum SmartSubscriptType: String {
...
class MonochromeThen: NSObject {
    //: static let `default` = SVGAEffectTool()
    static let `default` = MonochromeThen()
    ...
    private lazy var AppResPath: String = {
        ...
        let res_Dir = (docuPath as NSString).appendingPathComponent(a_bottomName)
        //: let version = Defaults.string(forKey: AppResourcesVersionKey) ?? ""
        let version = a_collectionName.string(forKey: a_itemName) ?? ""
        //: if version == AppVersion, FileManager.default.fileExists(atPath: res_Dir) {
        if version == a_askValue, FileManager.default.fileExists(atPath: res_Dir) {
            //: return res_Dir
            return res_Dir
        }
        //: let path = PodspecBundle.bundle.path(forResource: PodspecName, ofType: ".zip") ?? ""
        let path = LimitPodspecBundle.bundle.path(forResource: a_bottomName, ofType: (String(str_labValue.prefix(4)))) ?? ""
        ...
        let zipSuc = SSZipArchive.unzipFile(atPath: path,
                                            ...
                                            password: a_bottomName,
                                            //: progressHandler: nil)
                                            progressHandler: nil)
        //: if zipSuc {
        if zipSuc {
            //: Defaults.set(AppVersion, forKey: AppResourcesVersionKey)
            a_collectionName.set(a_askValue, forKey: a_itemName)
            //: return res_Dir
            return res_Dir
        }
        //: return ""
    ...
    private lazy var imageResPath: String = //: return (AppResPath as NSString).appendingPathComponent("pic")
        (AppResPath as NSString).appendingPathComponent((str_appOfName.replacingOccurrences(of: "voice", with: "c")))
    //: }()

    //: private lazy var languageImageResPath: String = {
    private lazy var languageImageResPath: String = {
        //: let language = LanguageManager.shared.currLanguage
        let language = ConstraintLanguageManager.shared.currLanguage
    ...
    private lazy var norImageResPath: String = {
        ...
        return (imageResPath as NSString).appendingPathComponent((String(str_objectValue)))
        //: }()
    }()
    ...
    private lazy var svgaResPath: String = //: return (AppResPath as NSString).appendingPathComponent("svga")
        (AppResPath as NSString).appendingPathComponent((String(str_defineName)))
    //: }()
    ...
    private lazy var mp3ResPath: String = //: return (AppResPath as NSString).appendingPathComponent("mp3")
        (AppResPath as NSString).appendingPathComponent((str_aspectTitle.replacingOccurrences(of: "birthday", with: "mp")))
    //: }()
    ...
    public lazy var modelResPath: String = //: return (AppResPath as NSString).appendingPathComponent("model")
        (AppResPath as NSString).appendingPathComponent((String(str_wrapValue)))
    //: }()
}

//: extension SVGAEffectTool {
extension MonochromeThen {
    /// 直接获取图片
    //: func getImage(name: String) -> UIImage? {
    func sessionText(name: String) -> UIImage? {
        //: let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending("@2x.png")
        let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending((str_giftGiveData.capitalized + String(str_objectName.suffix(5))))
        ...
        let path = (norImageResPath as NSString).appendingPathComponent(name).appending((str_giftGiveData.capitalized + String(str_objectName.suffix(5))))
    ...
    func kindPlus(name: String, type: String = (str_aspectTitle.replacingOccurrences(of: "birthday", with: "mp"))) -> String {
