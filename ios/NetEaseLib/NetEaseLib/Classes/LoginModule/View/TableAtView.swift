...
fileprivate let str_iconContent:[UInt8] = [0x69,0x6e,0x69,0x74,0x28,0x63,0x6f,0x64,0x65,0x72,0x3a,0x29,0x20,0x68,0x61,0x73,0x20,0x6e,0x6f,0x74,0x20,0x62,0x65,0x65,0x6e,0x20,0x69,0x6d,0x70,0x6c,0x65,0x6d,0x65,0x6e,0x74,0x65,0x64]

/*: "8075F5" :*/
fileprivate let str_topValue:[Character] = ["8","0","7","5","F","5"]

/*: "9A9B9C" :*/
fileprivate let str_viewText:String = "automatic if var9A9B9C"

/*: "kOpacityAnimation" :*/
fileprivate let str_afterContent:[Character] = ["k","O","p","a","c","i","t","y"]
fileprivate let str_heatGenderValue:[Character] = ["A","n","i","m","a","t","i","o","n"]

/*: "opacity" :*/
fileprivate let str_offValue:[Character] = ["o","p","a","c","i","t"]
fileprivate let str_imageValue:String = "info"

/*: "forwards" :*/
fileprivate let str_tableName:String = "FORW"

/*: "easeIn" :*/
fileprivate let str_sizeName:String = "view"
fileprivate let str_toValue:[Character] = ["a","s","e","I","n"]

/*: "8C7AF8" :*/
fileprivate let str_imageName:[Character] = ["8","C","7","A","F","8"]
...
class TableAtView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_iconContent, encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(paintTheLily(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension TableAtView {
    ...
    func model() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(str_topValue)))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (String(str_viewText.suffix(6))))!.withAlphaComponent(0.25).cgColor
    ...
    fileprivate func conversation(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(str_afterContent) + String(str_heatGenderValue)))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(betweenBackgroundAnimation(), forKey: (String(str_afterContent) + String(str_heatGenderValue)))
        }
    ...
    fileprivate func betweenBackgroundAnimation() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (String(str_offValue) + str_imageValue.replacingOccurrences(of: "info", with: "y")))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (str_tableName.lowercased() + "ards"))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (str_sizeName.replacingOccurrences(of: "view", with: "e") + String(str_toValue))))
        //: return animation
        return animation
    }
}
...
extension TableAtView {
    ...
    private func dataTo() {
        ...
        for index in 0 ..< codeNumber {
            ...
            colorAwakeStigmatize(label: label)
            ...
            line.fillColor = UIColor(hex: (String(str_imageName)))?.cgColor
