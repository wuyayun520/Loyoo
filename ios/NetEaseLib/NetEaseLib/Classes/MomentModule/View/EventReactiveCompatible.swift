...
class EventReactiveCompatible: UIView {
	var withOn: Bool = false
	var stochasticProcessName: String = "back"

    var makeThanImageView: UIImageView?
    ...
    override func draw(_ rect: CGRect) {
        ...
        path.stroke()
            ...
            self.withOn = layerInviteOpen
            
                self.withOn = true
                self.withOn = !self.withOn
            return self.withOn
            }
            rowChoiceLet.extraContent = { [self] colorContent in
            self.stochasticProcessName = colorContent
            
            return self.stochasticProcessName
            }
...
extension EventReactiveCompatible {
    ...
    func supplementOn(percent: CGFloat) {
