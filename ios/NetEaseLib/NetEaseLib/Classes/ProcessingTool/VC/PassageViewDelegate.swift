...
fileprivate let str_objectTopData:[UInt8] = [0x69,0x6e,0x69,0x74,0x28,0x63,0x6f,0x64,0x65,0x72,0x3a,0x29,0x20,0x68,0x61,0x73,0x20,0x6e,0x6f,0x74,0x20,0x62,0x65,0x65,0x6e,0x20,0x69,0x6d,0x70,0x6c,0x65,0x6d,0x65,0x6e,0x74,0x65,0x64]

/*: "border_window_picture" :*/
fileprivate let str_viewText:[Character] = ["b","o","r","d","e","r","_","w","i","n","d","o","w","_","p","i","c","t"]
fileprivate let str_nameValue:String = "userre"

/*: "Post" :*/
fileprivate let str_conversationRoomTitle:String = "view normal request in socialPost"

/*: "Profile" :*/
fileprivate let str_styleTapValue:[Character] = ["P","r","o","f","i","l","e"]

/*: "bth_me_follow_nor" :*/
fileprivate let str_stopValue:[Character] = ["b","t","h","_","m","e","_","f","o","l","l"]
fileprivate let str_showContent:[Character] = ["o","w","_","n","o","r"]

/*: "party_mini_bg_userDetail" :*/
fileprivate let str_betweenSizeData:[Character] = ["p","a","r"]
fileprivate let str_modelData:[Character] = ["t","y","_","m","i","n","i","_","b","g","_","u","s","e","r","D","e","t","a","i","l"]

/*: "removeAttentionUid" :*/
fileprivate let str_assetTitle:[Character] = ["r","e","m","o","v","e","A"]
fileprivate let str_containerValue:String = "tthiddennt"

/*: "attentionUid" :*/
fileprivate let str_fatalName:String = "textt"
fileprivate let str_cornerContent:[Character] = ["t","e","n","t","i","o","n","U","i","d"]

/*: "bth_me_follow_pre" :*/
fileprivate let str_listTitle:[UInt8] = [0x62,0x74,0x68,0x5f,0x6d,0x65,0x5f,0x66,0x6f,0x6c,0x6c,0x6f,0x77,0x5f,0x70,0x72,0x65]
...
class MediaReactiveCompatible: NSObject {
...
class PassageViewDelegate: PropertyViewController {
	var actionTotal: Double = -51.1
	var clickContent: String = "title"
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_objectTopData, encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        tableImageView = UIImageView(frame: self.view.bounds.integral)
        if let tableImageView = tableImageView {
            self.tableImageView?.image = UIImage(named: (String(str_viewText) + str_nameValue.replacingOccurrences(of: "user", with: "u")))
            if (tableImageView.sizeThatFits(.zero).width == 80.56) && (tableImageView.constraintsAffectingLayout(for: .vertical).count == 85) {
                //: SWIFT_CUSTOM_DANGER
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        ofData()
            ...
            if (bottomView.layer.anchorPoint.y != 0.5) && (bottomView.motionEffects.count == 12) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let successNumber = TalkView(frame: bottomView.frame.union(CGRect(x: CGFloat(457.47), y: CGFloat(384.65), width: CGFloat(0), height: CGFloat(103.50))))
            successNumber.imageAcrossClose = userInfoModel.isInRoom

            
            successNumber.blockDataNumber = { [self] imaginationImageTotal in
            self.actionTotal = imaginationImageTotal
            
                self.actionTotal += 1
                if self.actionTotal < 39 {
                    self.actionTotal = self.actionTotal - 1
                }
            return self.actionTotal
            }
            successNumber.commentGiftName = { [self] dataServiceContent in
            self.clickContent = dataServiceContent
            
            var successNumber = self.uid
            let scan = successNumber.prefix(through: successNumber.index(successNumber.startIndex, offsetBy: (successNumber.dropFirst().isEmpty ? 4 : 0))).isEmpty
            successNumber.removeAll(keepingCapacity: scan)
            if successNumber.hasPrefix(self.clickContent) {
                self.clickContent = successNumber
            }
            
            if let margin = self.clickContent.last {
                if self.clickContent.dropFirst(5).contains(margin) {
                    NotificationCenter.default.post(name: NSNotification.Name("choice"), object: self.clickContent.hasPrefix(self.clickContent.lowercased() + "position"))
                }
            }
            return self.clickContent
            }
                bottomView.addSubview(successNumber)
            }
    ...
    private lazy var navView: ToBarReactiveCompatible = { // 导航
        //: let v = TalkingUserDetailNaviBarView()
        let v = ToBarReactiveCompatible()
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var headerView: MView = { // banner
    ...
    private lazy var bottomView: HalogenBottomView = { // bottom
        //: let v = TalkingUserDetailBottomView(frame: .zero)
        let v = HalogenBottomView(frame: .zero)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var segmentedView: JXSegmentedView = {
        ...
        let segmentedView = JXSegmentedView(frame: CGRect(x: 0, y: a_limitRecordScreenName, width: a_blockValue, height: 44))
        ...
        segmentedDataSource.titleNormalColor = UIColor.observerShared()
        //: segmentedDataSource.titleSelectedColor = UIColor.appTitleColor()
        segmentedDataSource.titleSelectedColor = UIColor.blockOf()
        ...
        indicator.indicatorColor = UIColor.blockOf()
    ...
    private lazy var pagerView: JXPagingListRefreshView = {
        ...
        view.pinSectionHeaderVerticalOffset = Int(a_itemTitle)
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var titleArray: [String] = //: return ["Post".localized, "Profile".localized]
        [(String(str_conversationRoomTitle.suffix(4))).localized, (String(str_styleTapValue)).localized]
    //: }()

    //: private lazy var dataVC: [TalkingBaseViewController] = {
    private lazy var dataVC: [PropertyViewController] = //: return [postVC, profileVC]
        [postVC, profileVC]
    //: }()

    //: private lazy var postVC: TalkingUserDetailPostViewController = {
    private lazy var postVC: ImageEndUserDataSource = {
        //: let vc = TalkingUserDetailPostViewController()
        let vc = ImageEndUserDataSource()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var profileVC: ChartObjectProtocol = {
        //: let vc = TalkingUserDetailProfileViewController()
        let vc = ChartObjectProtocol()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var followBtn: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.outsideText(name: "bth_me_follow_nor")
        img.image = UIImage.outsideText(name: (String(str_stopValue) + String(str_showContent)))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(followButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(postWithClick))
    ...
    private lazy var voiceRoomBtn: UIButton = {
        ...
        voiceRoomBtn.isHidden = true
        //: voiceRoomBtn.setBackgroundImage(UIImage.outsideText(name: "party_mini_bg_userDetail"), for: .normal)
        voiceRoomBtn.setBackgroundImage(UIImage.outsideText(name: (String(str_betweenSizeData) + String(str_modelData))), for: .normal)
        //: return voiceRoomBtn
        return voiceRoomBtn
        //: }()
    }()
...
extension PassageViewDelegate {
    ...
    private func presentRequest() {
        ...
        if userInfoModel.isAttention {
            ...
            dict[(String(str_assetTitle) + str_containerValue.replacingOccurrences(of: "hidden", with: "e") + "ionUid")] = userInfoModel.uid
            //: isattion = false
            isattion = false
            //: } else {
        } else {
            //: dict["attentionUid"] = userInfoModel.uid
            dict[(str_fatalName.replacingOccurrences(of: "text", with: "a") + String(str_cornerContent))] = userInfoModel.uid
        }
    ...
    func ofData() {
        ...
        queue.async(group: group) {
            ...
            ConsumerRequestManager.emptyEach(uid: self.uid) { succeed, result, _ in
                //: self.setNoremarlView()
                self.serration()
                //: if self.uid == MeasurementAppManager.share.loginUid {
                if self.uid == MeasurementAppManager.share.loginUid {
                ...
                if succeed == true {
                    ...
                    if let userModel = JSONDeserializer<ToInfoModel>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        ...
                        self.navView.infoFile(userModel)
                        //: self.headerView.refreshView(userModel)
                        self.headerView.cancelModel(userModel)
                        //: self.bottomView.refreshView(userModel)
                        self.bottomView.facility(userModel)
                        //: self.postVC.refreshPostViewController(userModel)
                        self.postVC.controller(userModel)
                        ...
                        self.roomHeadImgView.posit(urlStr: userModel.headPic, options: optionsInfo)
                    }
                }
                //: group.leave()
                group.leave()
        ...
        queue.async(group: group) {
            ...
            ConsumerRequestManager.halogen(uid: self.uid) { succeed, result, _ in
                //: if succeed == true {
                if succeed == true {
                    ...
                    for dic in result as! [[String: Any]] {
                        ...
                        if let model = ValueTransformable.deserialize(from: dic as? Dictionary) {
    ...
    private func notifyOpen() {
        ...
        updatefollowAndStatus()

        // profile分区数据
        //: var profileSectionArr = [TUserDetailCellType.profileType]
        var profileSectionArr = [EqualTrivialityCellType.profileType]
        ...
        self.profileVC.downward(giftsArr, profileSectionArr, userInfoModel)
    }
}
...
extension PassageViewDelegate {
    ...
    @objc private func postWithClick() {
        //: req_followRequest()
        presentRequest()
    }
    ...
    private func updatefollowAndStatus() {
        //: self.followBtn.isHidden = (userInfoModel.uid == MeasurementAppManager.share.loginUid)
        self.followBtn.isHidden = (userInfoModel.uid == MeasurementAppManager.share.loginUid)
        //: let followImgStr = self.userInfoModel.isAttention ? "bth_me_follow_pre":"bth_me_follow_nor"
        let followImgStr = self.userInfoModel.isAttention ? String(bytes: str_listTitle, encoding: .utf8)! : (String(str_stopValue) + String(str_showContent))
        //: self.followBtn.image = UIImage.outsideText(name: followImgStr)
        self.followBtn.image = UIImage.outsideText(name: followImgStr)
    }
    ...
    private func finish() {
        ...
        guard userInfoModel.loungePlus else { return }
        //: guard let vipModel = MeasurementAppManager.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userInfoModel.vipSkinId}).first else { return }
        guard let vipModel = MeasurementAppManager.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userInfoModel.vipSkinId }).first else { return }
        ...
        vipCenterView.recordFinish(urlStr: vipModel.vipSkinCenter, placeImg: nil)
        //: vipBottomView.setUrlImage(urlStr: vipModel.vipSkinBottom, placeImg: nil)
        vipBottomView.recordFinish(urlStr: vipModel.vipSkinBottom, placeImg: nil)
    }
}
...
extension PassageViewDelegate: JXSegmentedViewDelegate {
...
extension PassageViewDelegate: JXPagingViewDelegate {
...
extension PassageViewDelegate {
    ...
    private func serration() {
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(a_itemBarValue)
        }
    }

    /// 添加约束
    //: private func setupSubViewsConstraint() {
    private func modelRestriction() {
        //: pagerView.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight)
        pagerView.frame = CGRect(x: 0, y: 0, width: a_blockValue, height: a_formalValue)
        ...
        voiceRoomBtn.snp.makeConstraints { make in
            ...
            make.bottom.equalToSuperview().offset(-actualHeight(h: 178) - a_agentData)
    ...
    private func capsule() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(stateLoad),
                                               //: name: USER_GETMYINFO_SUCCEED_NOTIFICATION,
                                               name: a_limitSuccessVideoValue,
                                               //: object: nil)
                                               object: nil)
    }
    ...
    private func atData() {
        ...
        voiceRoomBtn.rx.tap
            ...
            .subscribe(onNext: { [weak self] in
                //: guard let self = self else {return}
                guard let self = self else { return }
                //: LimitPushManager.share.func_pushToVoiceRoomVC(roomId: self.userInfoModel.partyRoomId.stringValue)
                LimitPushManager.share.sizeIn(roomId: self.userInfoModel.partyRoomId.stringValue)
                //: })
            })
