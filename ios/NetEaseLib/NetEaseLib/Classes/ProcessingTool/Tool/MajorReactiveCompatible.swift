...
fileprivate let str_actionTitle:[UInt8] = [0x54,0x68,0x69,0x73,0x20,0x66,0x65,0x61,0x74,0x75,0x72,0x65,0x20,0x69,0x73,0x20,0x6e,0x6f,0x74,0x20,0x61,0x76,0x61,0x69,0x6c,0x61,0x62,0x6c,0x65,0x20,0x64,0x75,0x72,0x69,0x6e,0x67,0x20,0x61,0x20,0x63,0x61,0x6c,0x6c]

/*: "wav" :*/
fileprivate let str_indexWhiteName:String = "equalav"
...
protocol LabToolDelegate: NSObject {
    /// 录音时间
...
class MajorReactiveCompatible: NSObject {
    //: open weak var delegate: TalkingAudioToolDelegate?
    open weak var delegate: LabToolDelegate?
    ...
    static let shared = MajorReactiveCompatible()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(dropWithRegard(notification:)), name: AVAudioSession.interruptionNotification, object: nil)
    }

    //: func getVoiceRecordLocalPathWithFileType(path: NSString) -> String {
    func report(path: NSString) -> String {
        //: if !FileManager.default.fileExists(atPath: Talking_Chat_record_SandboxPath) {
        if !FileManager.default.fileExists(atPath: a_successSessionName) {
            //: do {
            do {
                ...
                try FileManager.default.createDirectory(atPath: a_successSessionName, withIntermediateDirectories: true, attributes: nil)
                //: }catch {
            } catch {}
        }
        ...
        let timestr = NSDate.timeIntervalSinceReferenceDate
        //: let uid = MeasurementAppManager.share.loginUserMode.userID
        let uid = MeasurementAppManager.share.loginUserMode.userID
        //: if uid.count == 0 { return "" }
        if uid.count == 0 { return "" }

        //: return Talking_Chat_record_SandboxPath + "_" + uid + "_" + String(Int(timestr)) + "." + (path as String)
        return a_successSessionName + "_" + uid + "_" + String(Int(timestr)) + "." + (path as String)
    }
}

//: extension TalkingAudioTool: AVAudioRecorderDelegate, AVAudioPlayerDelegate {
extension MajorReactiveCompatible: AVAudioRecorderDelegate, AVAudioPlayerDelegate {
    /// 录制、播放过程中中断
    //: @objc func interruptionChange(notification: NSNotification) -> Void {
    @objc func dropWithRegard(notification: NSNotification) {
        ...
        if changeReason?.intValue ?? 0 == AVAudioSession.InterruptionType.began.rawValue {
            ...
            self.episode(showMsg: String(bytes: str_actionTitle, encoding: .utf8)!.localized)
            //: if self.isRecording {
            if self.isRecording {
                //: self.delegate?.func__recordFinishWithFile(filePath: self.filePath)
                self.delegate?.messagePath(filePath: self.filePath)
...
//                destroyRecordTimer()
                ...
                self.delegate?.master(status: .PlayerInterruption)
            }
        }
    }
    ...
    func audioPlayerDidFinishPlaying(_: AVAudioPlayer, successfully _: Bool) {
        ...
        self.delegate?.master(status: .PlayerFinish)
    }
}
...
extension MajorReactiveCompatible {
    ...
    func toLock() {
        ...
        let path = report(path: (str_indexWhiteName.replacingOccurrences(of: "equal", with: "w")) as NSString)
    ...
    func greetImage() {
        ...
        recordTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(post(time:)), userInfo: nil, repeats: true)
    ...
    @objc func post(time _: Timer) {
        ...
        self.delegate?.recordBlock(currentTime: interval)
    }
    ...
    func logTextPlay() {
        ...
        self.delegate?.clipTime(currentTime: currentTime)
    }
    ...
    func bindEdit() {
        ...
        playTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(emptyNow(time:)), userInfo: nil, repeats: true)
        //: if let curTime: Timer = recordTimer {
        if let curTime: Timer = recordTimer {
            //: RunLoop.main.add(curTime, forMode: .common)
            RunLoop.main.add(curTime, forMode: .common)
    ...
    @objc func emptyNow(time _: Timer) {
        //: if self.player != nil {
        if self.player != nil {
