...
fileprivate let str_firstName:[UInt8] = [0x69,0x6e,0x69,0x74,0x28,0x63,0x6f,0x64,0x65,0x72,0x3a,0x29,0x20,0x68,0x61,0x73,0x20,0x6e,0x6f,0x74,0x20,0x62,0x65,0x65,0x6e,0x20,0x69,0x6d,0x70,0x6c,0x65,0x6d,0x65,0x6e,0x74,0x65,0x64]

/*: "visualisation_img" :*/
fileprivate let str_shareNoteTrueData:[Character] = ["v","i","s","u","a","l","i","s","a","t","i","o","n","_","i"]
fileprivate let str_requestGestureTitleValue:[Character] = ["m","g"]

/*: "img_loading_lose" :*/
fileprivate let str_userValue:String = "var show toimg_"
fileprivate let str_toOfData:[Character] = ["l","o","s","e"]
...
class CustomReactiveCompatible: UICollectionViewCell {
	var textSum: Double = 39.1
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_firstName, encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        self.zoomScrollView.frame = CGRect(x: 10, y: 0, width: self.width - 20, height: self.height)
        barImageView = UIImageView(frame: self.bounds.standardized)
        if let barImageView = barImageView {
            self.barImageView?.image = UIImage(named: (String(str_shareNoteTrueData) + String(str_requestGestureTitleValue)))
            if (barImageView.bounds.size.height == 163.14) && ((barImageView.inputAssistantItem.leadingBarButtonGroups.count == 7) && (barImageView.inputAssistantItem.trailingBarButtonGroups.count == 8)) {
                //: SWIFT_CUSTOM_DANGER
        ...
        self.magnitude()
            ...
            if (!zoomScrollView.isOpaque) && (zoomScrollView.intrinsicContentSize.width == 304.41) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let convertLet = TimePeriodView()


            convertLet.labCount = { [self] detailInterval in
            self.textSum = detailInterval
            
            return self.textSum
            }
                zoomScrollView.addSubview(convertLet)
            }
...
extension CustomReactiveCompatible {
    ...
    private func bigness() {
        ...
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(actionTo))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(server)).then {
...
extension CustomReactiveCompatible {
    ...
    func equalView(imgModel: CellMeasurable) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(str_userValue.suffix(4)) + "loading_" + String(str_toOfData))), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.magnitude()
            }
        }
    ...
    func withTapUrl(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(str_userValue.suffix(4)) + "loading_" + String(str_toOfData))), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.magnitude()
        }
    }
...
extension CustomReactiveCompatible: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension CustomReactiveCompatible {
    ...
    func magnitude() {
        ...
        var newW = a_blockValue
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = a_blockValue * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > a_formalValue {
            //: newH = ScreenHeight
            newH = a_formalValue
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * a_formalValue / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (a_blockValue - newW) / 2.0, y: (a_formalValue - newH) / 2)
