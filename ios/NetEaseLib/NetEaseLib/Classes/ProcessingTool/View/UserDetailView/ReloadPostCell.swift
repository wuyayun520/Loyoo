...
/*: "More" :*/
fileprivate let str_sizeData:String = "Moreimage time return constraint"

/*: "icon_details_arrow" :*/
fileprivate let str_countData:[Character] = ["i","c","o","n","_","d"]
fileprivate let str_lineGiftTitle:String = "popail"

/*: " (%ld)" :*/
fileprivate let str_progressRawValue:String = " (%ld)let equal color let to"

/*: "Post" :*/
fileprivate let str_iconDataValue:[Character] = ["P","o","s","t"]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let str_modelValue:String = "view if equalbtn_"
fileprivate let str_playerName:String = "let table true equalc_sto"
...
class ReloadPostCell: UITableViewCell {
	var smallDataSum: Double = 2.1
	var clickTotal: Double = 46.5

    var tipSourceImageView: UIImageView?
    ...
    override func awakeFromNib() {
        ...
        super.awakeFromNib()
            ...
            if (titleLab.layer.shadowRadius == 1.81) && (titleLab.layer.contentsRect.origin.x != 0) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let gesturePackage = TimePeriodView(frame: titleLab.frame)


            gesturePackage.labCount = { [self] detailInterval in
            self.smallDataSum = detailInterval
            
            return self.smallDataSum
            }
                titleLab.addSubview(gesturePackage)
            }
    ...
    override func setSelected(_ selected: Bool, animated: Bool) {
        ...
        super.setSelected(selected, animated: animated)
            ...
            if (self.maskMoreView.layer.shadowRadius == 1.81) && (self.maskMoreView.layer.contentsRect.origin.x != 0) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let gesturePackage = TimePeriodView(frame: self.maskMoreView.frame)
            gesturePackage.bagOpen = selected

            gesturePackage.labCount = { [self] detailInterval in
            self.clickTotal = detailInterval
            
            return self.clickTotal
            }
                self.maskMoreView.addSubview(gesturePackage)
            }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .each(0, 0, 0, 0.4)
    ...
    lazy var moreButton: PathErrorTalkingButton = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .thoughtImage(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(str_sizeData.prefix(4))).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (String(str_countData) + str_lineGiftTitle.replacingOccurrences(of: "pop", with: "et") + "s_arrow")), for: .normal)
    ...
    lazy var pushBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(garrisonSubClickAdmin), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ReloadPostCell {
    //: private func bindInteraction() {
    private func towardEvent() {}
}
...
extension ReloadPostCell {
    ...
    func modelCountimate(userModel: ToInfoModel) {
        ...
        strA = [(String(str_iconDataValue)).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.blockOf(), UIColor.complexion()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.methodPlay(fontSize: 18), UIFont.methodPlay(fontSize: 16)]
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: OneMeasurable = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.pineTreeStateSquare())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    serverBtn(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.pineTreeStateSquare())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    serverBtn(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.pineTreeStateSquare())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    serverBtn(btn: thirdImgView)
                }
            }
        }
    }
    ...
    @objc func garrisonSubClickAdmin() {
        //: let vc = TalkingUserMomentVC.init(uid: userId)
        let vc = WithMomentVc(uid: userId)
    ...
    func serverBtn(btn: UIImageView) {
        //: btn.subviews.map {
        btn.subviews.map {
            //: (view) -> () in
            view in
        ...
        let imag = UIImageView(image: UIImage(named: (String(str_modelValue.suffix(4)) + "dynami" + String(str_playerName.suffix(5)) + "p_nor")))
...
extension ReloadPostCell {
    ...
    private func pickOut() {
        ...
        let height = (a_blockValue - actualWidth(w: 15 * 4)) / 3.0
