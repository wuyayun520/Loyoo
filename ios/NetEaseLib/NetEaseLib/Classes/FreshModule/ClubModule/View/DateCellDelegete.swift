...
fileprivate let str_upName:[UInt8] = [0x69,0x6e,0x69,0x74,0x28,0x63,0x6f,0x64,0x65,0x72,0x3a,0x29,0x20,0x68,0x61,0x73,0x20,0x6e,0x6f,0x74,0x20,0x62,0x65,0x65,0x6e,0x20,0x69,0x6d,0x70,0x6c,0x65,0x6d,0x65,0x6e,0x74,0x65,0x64]

/*: "img_home_shadow" :*/
fileprivate let str_wrapData:String = "img_hopop push view line text"
fileprivate let str_userText:[Character] = ["m","e"]
fileprivate let str_addBackgroundValue:[Character] = ["_","s","h","a","d","o","w"]

/*: "#9610FF" :*/
fileprivate let str_bottomText:String = "model"
fileprivate let str_coverName:String = "you cat view9610FF"

/*: "#8566FF" :*/
fileprivate let str_viewTitle:[Character] = ["#","8","5","6","6","F"]
fileprivate let str_nameData:String = "away"

/*: "Random Video" :*/
fileprivate let str_equalMediumValue:String = "Randommake lab player image"
fileprivate let str_viewData:String = "EO"

/*: "icon_match_call_free" :*/
fileprivate let str_equalValue:String = "icon_view fill false as name"
fileprivate let str_collectionName:String = "_caimport fatal result white data"
fileprivate let str_layerName:[Character] = ["l","l","_","f","r","e","e"]

/*: "Free" :*/
fileprivate let str_normalName:String = "Freebutton view"

/*: "Scroll_upto_discove" :*/
fileprivate let str_cellText:[Character] = ["S","c","r","o","l","l","_"]
fileprivate let str_personData:String = "upto_dcall view manager model type"

/*: "icon_VectorStroke" :*/
fileprivate let str_addName:[Character] = ["i","c","o","n","_","V","e","c","t","o","r","S","t","r","o","k","e"]

/*: "get json error" :*/
fileprivate let str_requestContent:[Character] = ["g"]
fileprivate let str_hiddenData:[Character] = ["e","t"," ","j","s","o","n"," ","e","r","r","o","r"]
...
@objc protocol AddViewDelegate: NSObjectProtocol {
    ...
    @objc optional func contributionSize(cardView: DateCellDelegete, cardViewCell: SocialViewCell, forRowAtIndex: Int)

    //: @objc optional func didRemoveLastCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func ofIcon(cardViewCell: SocialViewCell, forRowAtIndex: Int)

    //: @objc optional func didDisplayCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func changeInfo(cardViewCell: SocialViewCell, forRowAtIndex: Int)

    //: @objc optional func didMoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell, forMovePoint: CGPoint)
    @objc optional func presentEqual(cardView: DateCellDelegete, cardViewCell: SocialViewCell, forMovePoint: CGPoint)
    /// 随机匹配视频
    //: @objc optional func clubCardRandowBtnClick()
    @objc optional func verbalise()
}

//: class TalkingClubCardView: UIView {
class DateCellDelegete: UIView {
	var compartmentTitle: String = "enter"
	var sizeInformationDictionary: [AnyHashable: String] = [:]

    var hustleDataImageView: UIImageView?
    ...
    var maxRemoveDistance = a_blockValue / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [SocialViewCell]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: GestureObjectProtocol? /// 数据源
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_upName, encoding: .utf8)!)
    }
    ...
    override func didMoveToSuperview() {
        ...
        sizeCreate()
            ...
            if (freeImag.alignmentRectInsets.bottom == 11) && (freeImag.convert(CGRect(x: 0, y: 0, width: CGFloat(144.74), height: 0), to: freeImag.superview).origin.x == 25.95) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let userCourse = EndTapView(frame: freeImag.bounds.intersection(CGRect(x: CGFloat(0), y: CGFloat(607.97), width: CGFloat(155.26), height: CGFloat(3.71))))
            userCourse.targetTotal = maxAngle
            userCourse.indexClickName = { [self] popName in
            self.compartmentTitle = popName
            
            var userCourse = self.identifier
            userCourse = String(userCourse.sorted(by:<))
            if userCourse.hasPrefix(self.compartmentTitle) {
                self.compartmentTitle = userCourse
            }
            
            return self.compartmentTitle
            }
            userCourse.sexAtDictionary = { [self] moveDictionary in
            self.sizeInformationDictionary = moveDictionary
            
            guard var value = self.sizeInformationDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                freeImag.addSubview(userCourse)
            }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.contentMode = .scaleAspectFill
        //: imag.image = UIImage.outsideText(name: "img_home_shadow")
        imag.image = UIImage.outsideText(name: (String(str_wrapData.prefix(6)) + String(str_userText) + String(str_addBackgroundValue)))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.stopAcross(colors: [UIColor(hex: (str_bottomText.replacingOccurrences(of: "model", with: "#") + String(str_coverName.suffix(6))))!.cgColor, UIColor(hex: (String(str_viewTitle) + str_nameData.replacingOccurrences(of: "away", with: "F")))!.cgColor], size: CGSize(width: a_blockValue - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(str_equalMediumValue.prefix(6)) + " Vid" + str_viewData.lowercased()).localized, for: .normal)
        //: btn.addTarget(self, action: #selector(randomBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(organize), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
    ...
    lazy var freeImag: UIButton = {
        //: let img = UIButton()
        let img = UIButton()
        //: img.setBackgroundImage(UIImage.outsideText(name: "icon_match_call_free"), for: .normal)
        img.setBackgroundImage(UIImage.outsideText(name: (String(str_equalValue.prefix(5)) + "match" + String(str_collectionName.prefix(3)) + String(str_layerName))), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(str_normalName.prefix(4))).localized, for: .normal)
...
extension DateCellDelegete {
    ...
    private func withPage() {
        ...
        image.contentMode = .scaleAspectFill
        //: image.image = UIImage.outsideText(name: "Scroll_upto_discove")
        image.image = UIImage.outsideText(name: (String(str_cellText) + String(str_personData.prefix(6)) + "iscove"))
        ...
        image2.contentMode = .scaleAspectFill
        //: image2.image = UIImage.outsideText(name: "icon_VectorStroke")
        image2.image = UIImage.outsideText(name: (String(str_addName)))
    ...
    private func want(player: SVGAPlayer) {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Club_guidePage)
            let url = MonochromeThen.default.telecasting(type: .Club_guidePage)
            ...
            printLog(message: (String(str_requestContent) + String(str_hiddenData)))
        }
    }
    ...
    @objc private func organize() {
        ...
        self.delegate?.verbalise?()
    }

    //: func reloadDataAnimated(anim: Bool) {
    func methodTo(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.colorLayer(cardView: self) ?? 0
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? SocialViewCell(), at: 0)
    ...
    func panoptical(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: SocialViewCell = self.visibleCells[i] as! SocialViewCell
            ...
            if isVisbleLast {
                ...
                self.delegate?.changeInfo?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                totaleractionCell(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func totaleractionCell(cell: SocialViewCell, transform: CGAffineTransform) {
    ...
    func modelCamera(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func smartCountimate(identifier: String) -> SocialViewCell? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! ImageCompartmentViewCell.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: ImageCompartmentViewCell = cellclass.init(reuseIdentifier: identifier)
...
extension DateCellDelegete: UserCellDelegete {
    ...
    func hollerithCard(cell: SocialViewCell) {
        ...
        self.delegate?.contributionSize?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.colorLayer(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.ofIcon?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
...
extension DateCellDelegete {
    ...
    func deadlineTarget() {
        ...
        let index = UpMarkContentConvertible.randow
