...
fileprivate let str_hostName:[UInt8] = [0x69,0x6e,0x69,0x74,0x28,0x63,0x6f,0x64,0x65,0x72,0x3a,0x29,0x20,0x68,0x61,0x73,0x20,0x6e,0x6f,0x74,0x20,0x62,0x65,0x65,0x6e,0x20,0x69,0x6d,0x70,0x6c,0x65,0x6d,0x65,0x6e,0x74,0x65,0x64]

/*: "btn_me_edit" :*/
fileprivate let str_viewData:[Character] = ["b","t","n","_","m","e","_","e","d","i"]
fileprivate let str_streamData:[Character] = ["t"]

/*: "icon_me_lw" :*/
fileprivate let str_giftValue:[Character] = ["i","c","o","n","_","m","e","_","l","w"]

/*: " Reward" :*/
fileprivate let str_neverValue:String = " Rewaruser comb table"
fileprivate let str_timeData:[Character] = ["d"]

/*: "FF506D" :*/
fileprivate let str_afterText:String = "FF50nameD"
...
class LoadView: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_hostName, encoding: .utf8)!)
    }
    ...
    private lazy var editImag: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.outsideText(name: (String(str_viewData) + String(str_streamData))))
        //: return imag
        return imag
        //: }()
    }()
    ...
    private lazy var taskCenterBtn: UIButton = {
        ...
        let btn = UIButton()
        //: btn.setImage(UIImage.outsideText(name: "icon_me_lw"), for: .normal)
        btn.setImage(UIImage.outsideText(name: (String(str_giftValue))), for: .normal)
        //: btn.setTitle(" Reward".localized, for: .normal)
        btn.setTitle((String(str_neverValue.prefix(6)) + String(str_timeData)).localized, for: .normal)
        //: btn.setTitleColor(UIColor.init(hex: "FF506D"), for: .normal)
        btn.setTitleColor(UIColor(hex: (str_afterText.replacingOccurrences(of: "name", with: "6"))), for: .normal)
...
extension LoadView {
    ...
    func pathCreate(title: String, icon: String, row: Int, isLast: Bool) {
        //: titleBLB.text = title.localized
        titleBLB.text = title.localized
        //: iconImag.image = UIImage.outsideText(name: icon)
        iconImag.image = UIImage.outsideText(name: icon)
        //: index = row
        index = row
        //: islast = isLast
...
extension LoadView {
    ...
    private func withConstraint() {
        ...
        backView.frame = CGRect(x: 15, y: 0, width: a_blockValue - 30, height: backView_h)
        ...
        if index == 0, islast {
            ...
            backView.barRadii(width: backView.frame.size.width,
